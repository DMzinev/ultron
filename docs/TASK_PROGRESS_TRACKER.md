...
| 36 | **P3-E1** | `agent/P3-E1-security-hygiene-tests` | **COMPLETED** | `pending` | Fail-Closed Strict Rejection policy: null-byte injection guards, directory-traversal containment (`realpath` + `startswith`), root filesystem prohibition, Windows reserved device name shielding (`CON`/`PRN`/`AUX`/`NUL`/`COM1-9`/`LPT1-9`), symlink escape containment; hardened 3 route files (`browse_folder.py`, `analysis_routes.py`, `system_routes.py`); 11/11 adversarial tests in `test_path_security.py`; `docs/API_SURFACE.md` Section 4 documents policy; master gate passing (`TESTS: 812 ran, 0 failed, 0 errors, 9 skipped`). |
