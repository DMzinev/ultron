...
MCP_TOOLS = list(CANONICAL_TOOLS) + [
    {
        "name": "ultron_generate_fix",
        "description": "One-click fix mission compiler generating prompt envelope and interface signatures.",
        "inputSchema": {
            "type": "object",
            "properties": {
                "target_file": {"type": "string", "description": "Target file to fix"},
...
def _execute_tool(tool_name: str, arguments: dict, default_repo: str = ".") -> dict:
    ...
    if tool_name == "ultron_generate_fix":
    ...
    elif tool_name == "get_context_brief":
        from ultron.core.context_brief import generate_vibe_context_package
        intent = arguments.get("intent", "")
        repo = arguments.get("repo_path", default_repo or os.getcwd())
        ...
        abs_repo, abs_target, norm_rel = _resolve_paths(raw_path, arguments.get("repo_path", default_repo))
...
def handle_mcp_request(raw_line):
    ...
    if method == "initialize":
        ...
        try:
            ...
            if not result_payload.get("isError"):
                result_payload.pop("isError", None)
            return {
                "jsonrpc": "2.0",
                "id": req_id,
