...
async function scan() {
  ...
  try {
    const [overviewData, healthData, graphData] = await Promise.all([
      ...
      api("/api/dependency-graph", { repo }).catch(() => ({}))
    ]);
...
function renderTopologyGraph() {
    ...
    if (state.risks && state.risks.length > 0) {
      state.graphData = {
        nodes: state.risks.map((r) => ({
          id: r.file || r.file_path,
          label: splitPath(r.file || r.file_path).base,
          level: r.level || "LOW",
      ...
      state.risks.forEach((r) => {
        (r.callers || []).forEach((c) => {
          state.graphData.links.push({ source: c, target: r.file || r.file_path });
        });
      });
    } else {
      return;
  ...
  if (nodes.length > 90) {
    nodes = [...nodes].sort((a, b) => (b.impact_score || 0) - (a.impact_score || 0)).slice(0, 90);
  }
  ...
  graphSimulationNodes = nodes.map((n, i) => {
    ...
    return {
      ...
      radius: Math.min(16, Math.max(7, Math.sqrt(n.complexity || 5) * 2.2))
    };
  });
